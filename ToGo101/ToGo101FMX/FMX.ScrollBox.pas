...
{ Copyright(c) 2014-2025 Embarcadero Technologies, Inc. }
{              All rights reserved                      }
{                                                       }
{*******************************************************}
...
type
  ...
  published
    ...
    ///<summary>Enable or disable scroll</summary>
    property EnabledScroll: Boolean read FEnabledScroll write SetEnabledScroll;
    ///<summary>Enable or disable scrolling animation</summary>
  ...
  public
    ...
    ///<summary>Defines available scroll directions</summary>
...
const
  RightSide = [TAlignLayout.Top, TAlignLayout.TopLeft, TAlignLayout.TopCenter, TAlignLayout.TopRight, TAlignLayout.MostTop,
               TAlignLayout.Bottom, TAlignLayout.BottomLeft, TAlignLayout.BottomCenter, TAlignLayout.BottomRight, TAlignLayout.MostBottom];
  BottomSide = [TAlignLayout.Left, TAlignLayout.LeftTop, TAlignLayout.LeftCenter, TAlignLayout.LeftBottom, TAlignLayout.MostLeft,
                TAlignLayout.Right, TAlignLayout.RightTop, TAlignLayout.RightCenter, TAlignLayout.RightBottom, TAlignLayout.MostRight];
var
  I: Integer;
  R, LocalR: TRectF;
  Child: TControl;
begin
  ...
  begin
    Child := FContent.Controls[I];
    if Child.Visible then
    begin
      {$IFDEF MSWINDOWS}
      if (csDesigning in ComponentState) and Supports(Child, IDesignerControl) then
        Continue;
      {$ENDIF}
      // We are using this conversion instead of BoundsRect for applying possible Child Scale.
      LocalR.TopLeft := Child.ConvertLocalPointTo(Child.ParentControl, Child.LocalRect.TopLeft);
      LocalR.BottomRight := Child.ConvertLocalPointTo(Child.ParentControl, Child.LocalRect.BottomRight);
      if (Child.Align in RightSide) or (TAnchorKind.akRight in Child.Anchors) then
      begin
        LocalR.Left := R.Left;
        LocalR.Right := R.Right;
      end;
      if (Child.Align in BottomSide) or (TAnchorKind.akBottom in Child.Anchors) then
      begin
        LocalR.Top := R.Top;
        LocalR.Bottom := R.Bottom;
      end;
      if Child.Align in VerticalTopStackAlignments then
        LocalR.Bottom := LocalR.Bottom + Padding.Bottom;
      if Child.Align in HorizontalLeftStackAlignments then
...
function TCustomPresentedScrollBox.GetViewportRect: TRectF;
begin
  Result.TopLeft := ViewportPosition;
  Result.Size := ViewportSize;
end;
...
begin
  ...
  begin
    ViewportSize := ParentControl.BoundsRect.Size;
    Result.Width := Max(Result.Width, ViewportSize.Width);
    Result.Height := Max(Result.Height, ViewportSize.Height);
  end;
end;
...
function TScrollContent.IsVisibleChild(const AChild: TControl): Boolean;
var
  ViewportRect: TRectF;
  ChildBounds: TRectF;
begin
  Result := inherited;
  if Result and not (csDestroying in AChild.ComponentState) then
  begin
    ViewportRect := FScrollBox.ViewportRect;
    ChildBounds := AChild.BoundsRect;
    Result := ViewportRect.IntersectsWith(ChildBounds);
  end;
end;
...
function TScrollContent.PointInObjectLocal(X, Y: Single): Boolean;
  ...
  function IsInScrollBar(const AScrollBar: TScrollBar; const APoint: TPointF): Boolean;
  var
    ScrollBarRect: TRectF;
  begin
    if (AScrollBar = nil) or not AScrollBar.Visible then
      Exit(False);

    ScrollBarRect.TopLeft := ConvertLocalPointFrom(AScrollBar, TPointF.Zero);
    ScrollBarRect.size := AScrollBar.Size.Size;
    Result := ScrollBarRect.Contains(APoint);
  end;
...
begin
  ...
  HitPoint := TPointF.Create(X, Y);
  Result := LClipRect.Contains(HitPoint);
  Result := Result and not IsInScrollBar(ScrollBox.GetVScrollBar, HitPoint)
                   and not IsInScrollBar(ScrollBox.GetHScrollBar, HitPoint);
end;
...
procedure TScrollOptions.AssignTo(Dest: TPersistent);
var
  DestOptions: TScrollOptions;
begin
  if Dest is TScrollContentSize then
  begin
    DestOptions := TScrollOptions(Dest);
    DestOptions.FAutoHideScrollBars := AutoHideScrollBars;
    DestOptions.FBounces := Bounces;
    DestOptions.FEnabledScroll := EnabledScroll;
    DestOptions.FScrollAnimation := ScrollAnimation;
    DestOptions.FTouchTracking := TouchTracking;
    DestOptions.DoChanged;
  end
  else
    inherited;
end;

constructor TScrollOptions.Create(AOwner: TComponent; const AOnChange: TNotifyEvent);
begin
  inherited Create;
  if AOwner is TCustomPresentedScrollBox then
    FScrollBox := TCustomPresentedScrollBox(AOwner)
...
procedure TScrollOptions.DoChanged;
begin
  if Assigned(FOnInternalChange) then
    FOnInternalChange(Self);
end;

function TScrollOptions.GetOwner: TPersistent;
begin
  Result := FScrollBox;
end;

procedure TScrollOptions.SetAutoHideScrollBars(const Value: TBehaviorBoolean);
begin
  if FAutoHideScrollBars <> Value then
  begin
    FAutoHideScrollBars := Value;
    DoChanged;
  end;
end;

procedure TScrollOptions.SetBounces(const Value: TBehaviorBoolean);
begin
  if FBounces <> Value then
  begin
    FBounces := Value;
    DoChanged;
  end;
...
procedure TScrollOptions.SetEnabledScroll(const Value: Boolean);
begin
  if FEnabledScroll <> Value then
  begin
    FEnabledScroll := Value;
    DoChanged;
  end;
...
procedure TScrollOptions.SetScrollAnimation(const Value: TBehaviorBoolean);
begin
...
procedure TScrollOptions.SetTouchTracking(const Value: TBehaviorBoolean);
begin
